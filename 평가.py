...
def load_updated_weights(weights_file):
    ...
    with open(weights_file, 'r') as f:
        ...
        # 각 레이어의 가중치 개수 출력
        #print("가중치 파일에서 읽은 각 레이어의 가중치 개수:")
        #for i, layer in enumerate(weights):
        #    print(f"Layer {i + 1}: {layer.size} weights")
    return weights
...
def save_loaded_weights(weights, output_file):
    with open(output_file, 'w', encoding="utf-8") as f:
        for i, layer_weights in enumerate(weights):
            ...
            f.write('\n')
    # 각 레이어의 가중치 개수 출력
    #print("저장된 가중치 파일에서 각 레이어의 가중치 개수:")
    #for i, layer_weights in enumerate(weights):
    #    print(f"Layer {i + 1}: {layer_weights.size} weights")
...
# 모델 평가 함수
def evaluate_model(weights_file, base_path, csv_file, input_size, activation_function='relu', output_activation_function='softmax'):
    # 가중치 읽기
    weights = load_updated_weights(weights_file)

    # 레이어 구조 및 노드 수 출력
    layer_structure = [weights[i].shape for i in range(len(weights))]
    #print("레이어 구조 및 노드 수:")
    #for i, (rows, cols) in enumerate(layer_structure):
    #    print(f"레이어 {i + 1}: 입력 노드 수 = {cols}, 출력 노드 수 = {rows}")

    # 데이터셋 로드 및 각 문자 폴더에서 랜덤 샘플 선택
    dataset = load_dataset(base_path, csv_file)
    
    selected_samples = []
    for character in characters_list:
        character_images = [path for path in dataset.keys() if character in path]
        if character_images:
            selected_sample = random.choice(character_images)
            selected_samples.append(selected_sample)

    correct_predictions = 0
    total_samples = len(selected_samples)

    with open("C:/Users/asx12/OneDrive/바탕 화면/인공지능/label_comparison_output.txt", 'w', encoding="utf-8") as label_log:
        for sample in selected_samples:
            # 입력 데이터 및 타겟 가져오기
            input_data = read_bmp_image_1bit(os.path.join(base_path, sample), input_size)
            
            # 라벨 정보 가져오기
            target = dataset.get(sample, None)
            if target is None:
                # 라벨을 찾지 못한 경우 건너뜀
                continue
            ...
            if predicted_character == character_name:
                print(f"실제: {character_name}, 예측: {predicted_character} - 예측이 맞았습니다.")
                correct_predictions += 1
            else:
                print(f"실제: {character_name}, 예측: {predicted_character} - 예측이 틀렸습니다.")
...
def main():
    ...
    csv_file = "C:/Users/asx12/OneDrive/바탕 화면/인공지능/dataset_labels_ex.csv"
    input_size = 64 * 64
    
    # 가중치 읽기 및 평가
    evaluate_model(weights_file, base_path, csv_file, input_size, activation_function='relu', output_activation_function='softmax')
